...
from det3d.models.loss.chamferloss import chamfer_distance
...
class MotionHead(nn.Module):
    ...
    def loss(self, example, pred_motion, voxel_cfg, eps=1e-8, **kwargs):
        ...
        cam_id = example['points_cam_id']
        lidar_to_next_cam = example["lidar_to_next_cam"]
        next_cam_intrinsic = example['next_cam_intri']
        points_cam_coords = example["points_cam_coords"]
        flow = example['points_flow']
        ...
        for batchidx in range(batch_size):
            ...
            for camid in range(6):
                ...
                with torch.no_grad():
                    ...
                    ego_points_cam = ego_points_cam[:2] / (ego_points_cam[2:] + 1e-8)
                ...
                points_to_next_cam = points_to_next_cam[:2] / (points_to_next_cam[2:] + 1e-8)
    ...
    def predict(self, example, pred_motion):
        ret_list = []
        for i in range(len(meta)):
            ret_list.append(dict(token=example[i]['token'],
